...
/**
 * Runs the shared {@link BloomFilterTest} suite against {@link XORBlockedBloomFilter}, i.e.
 * checks that the {@code XOR} variant never produces a false negative and stays within the same
 * false-positive budget as {@link BlockedBloomFilter} at the same sizing - which it only does
 * because it reduces by a modulo rather than inheriting the multiply-shift.
 */
public class XORBlockedBloomFilterTest extends BloomFilterTest {
	@Override
	...
	protected ProbFilter createFilter(long size, double fpp) {
		...
		return isTestLarge() ? XORBlockedBloomFilter.newLargeBackedXOR(size, 10)
				: new XORBlockedBloomFilter(size, 10);
	}
