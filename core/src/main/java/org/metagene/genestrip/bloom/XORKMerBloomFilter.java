...
/**
 * The {@code XOR} variant of {@link KMerBloomFilter}: it hashes a k-mer by a bare exclusive or with the
 * hash function's factor, which is fast and apparently "good enough" for hashing k-mers, and pays for
 * it by reducing with a modulo instead of the multiply-shift of {@link KMerBloomFilter#reduce(long)}.
 * <p>
 ...
 * {@link KMerBloomFilter}'s 39 ns, i.e. some 15% less. The one-hash families hash only once, so there
 * the margin is small and depends on the backing - see {@link XORBlockedKMerBloomFilter}.
 */
public class XORKMerBloomFilter extends KMerBloomFilter {
     ...
     * fast.
     * <p>
     * <strong>This relies on {@link KMerBloomFilter#reduce(long)} being a modulo</strong>, i.e.
     ...
     * {@link KMerBloomFilter#reduce(long)}, because {@link #hash(long, int)} does not mix.
     * <p>
     ...
     * {@link BlockedKMerBloomFilter} and {@link SingleWordKMerBloomFilter} it derives nothing else from
     ...
     * is gone; that is what {@link MurmurKMerBloomFilter} is for. Should this ever be revisited, measure
