...
/**
 * File goal that writes, for each relevant parent node, a CSV file summarising the pairwise *k*-mer
 * intersection counts and Jaccard indices among the parent's child subtrees as computed by
 * {@link KMerIntersectCountGoal}. Each file lists the child count, *k*-mer sum, spread sum, average
 * spread and overspread ratio, followed by a matrix of intersection counts and a matrix of Jaccard
 * indices over the children plus the trailing "OTHER" slot. One file is produced per parent node found
 * at a refinement position (and per explicitly configured tax id).
 *
...
public class KMerIntersectCSVGoal<P extends FTProject> extends FileListGoal<P> {
    ...
    /**
     * Creates the goal under the {@link FTGoalKey#INTERSECT_CSV} key, depending on the intersection-count
     * goal.
     *
     * @param project           the FT project
     * @param storeGoal         the goal providing the loaded database (used to enumerate parent nodes)
     * @param kmerIntersectGoal the goal providing the accumulated per-parent intersection statistics
    ...
    /**
     * Reports this goal as already cleaned, since a real check would require making the goals this goal
     * depends on.
     *
    ...
    /**
     * Registers one output CSV file per parent node: every node matching a configured refinement
     * position (that carries stored *k*-mers) plus every explicitly configured tax id, recording the
     * file-to-node mapping used later by {@link #makeFile(File)}.
     */
    @Override
    ...
    /**
     * Writes the CSV report for the parent node mapped to the given file: a header row of summary
     * statistics, the child tax ids (with a trailing "OTHER" column), the full matrix of pairwise
     * intersection counts and finally the matrix of Jaccard indices.
     *
     * @param file the output file to write, previously registered by {@link #provideFiles()}
        ...
        try (PrintStream out = new PrintStream(StreamProvider.getOutputStreamForFile(file))) {
            ...
            for (int i = 0; i <= children.length; i++) {
                for (int j = 0; j <= children.length; j++) {
                    out.print(intersections.getIntersectionCount(parent, i, j, booleanConfigValue(FTConfigKey.WITH_DESCENDANT_COUNTS)));
                    out.print(';');
                }
                out.println();
            }
    ...
    /**
     * Collects the taxonomy nodes eligible to receive a CSV report: those matching one of the given
     * refinement positions that have at least one child node and whose tax id has a positive *k*-mer
     * count in the database statistics.
     *
     * @param database            the loaded database providing the tax tree and per-tax-id statistics
     * @param refinementIntervals the refinement positions a node must match to be included
     * @return the matching parent nodes
     */
