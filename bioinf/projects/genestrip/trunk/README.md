...
`cd` to the installation directory `genestrip`. Given a matching Maven and JDK installation, `mvn install` will compile, test and install the Genestrip program library. Afterwards a self-contained and executable `genestrip.jar` file will be stored under `./lib`. 
...
1. Create a folder `<project_name>` under `./data/projects/`. The folder is the place for all subsequent files that specify the content of the database to be built. It is also the core name of the database.
1. Create a text file `./data/projects/<project_name>/taxids.txt` with one tax id per line. The database will *only* contain *k*-mers from genomes that belong to tax ids from the file `taxids.txt`. If `taxids.txt` contains a tax id from a non-leaf node of the [taxonomy](https://ftp.ncbi.nlm.nih.gov/pub/taxonomy/taxdmp.zip), then all subordinate tax ids and *k*-mers from respective [RefSeq](https://ftp.ncbi.nlm.nih.gov/refseq/release/) genomes will be included in the database.
1. Create a text file `./data/projects/<project_name>/categories.txt`. The file tells Genestrip, which categories of organisms should be *considered* when generating the database and when determining a least common ancestor tax id for *k*-mer in the finalized database. You also have to ensure that the categories from `categories.txt` comprise all of the tax ids from `taxids.txt`: Only genomes of comprised tax ids will be used to generate the database.
...
```
The resulting CSV file will be named `human_virus.demo.fastq.gz_match.csv`, is stored in `./data/projects/human_virus/csv` and should have the following content:
```
```
The same principles apply for your own project with the name `<projec_name>` under `./data/projects`.
...
usage: genestrip [options] <project> [<goal1> <goal2>...]
 -d <path>     Base directory for all data files. The default is
               './data'.
 ...
 -r <path>     Common store folder for filtered fastq files and csv files
               ...
               '<base directory>/projects/<project name>/fastq' and '<base
